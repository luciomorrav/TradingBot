    ...
    async def connect(self):
        ...
        if self._private_key and not self._private_key.startswith("${"):
            try:
                ...
                # Funder = proxy wallet address (holds USDC). Required for proxy wallets.
                funder = self.config.get("funder_address") or None
                ...
                if (self._api_secret and not self._api_secret.startswith("${")
                    ...
                    logger.info("Polymarket CLOB client initialized (L2 auth, sig_type=%d, funder=%s)", sig_type, funder or "EOA")
                else:
